...
const cacheTitle = `ShareGlimmer-cache-${cacheVersion}`;
const urlsToCache = [
    'index.html',
    './',
    'styles.css',
...
const evaluateRequestData = function (request) {
    return new Promise(async (resolve) => {
        ...
        const ShareGlimmerUrl = request.url;
        ...
        if (files && files.length > 0) {
            ...
            const DBOpenRequest = indexedDB.open('ShareGlimmer_store');
            DBOpenRequest.onsuccess = e => {
                ...
                for (let i = 0; i < fileObjects.length; i++) {
                    ...
                    objectStoreRequest.onsuccess = _ => {
                        if (i === fileObjects.length - 1) resolve(ShareGlimmerUrl + '?share-target=files');
                    }
                }
            }
            DBOpenRequest.onerror = _ => {
                resolve(ShareGlimmerUrl);
            }
        } else {
            ...
            resolve(ShareGlimmerUrl + urlArgument);
        }
    });
}
