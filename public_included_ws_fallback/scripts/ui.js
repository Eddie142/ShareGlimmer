...
class PeersUI {
    ...
    _activatePasteMode(files, text) {
        if (!window.pasteMode.activated && (files.length > 0 || text.length > 0)) {
            ...
            if (files.length === 1) {
                ...
                noPeersMessage = `Open ShareGlimmer on other devices to send<br><i>${descriptor}</i>`;
            } else if (files.length > 1) {
                descriptor = `${files[0].name} and ${files.length-1} other files`;
                noPeersMessage = `Open ShareGlimmer on other devices to send<br>${descriptor}`;
            } else {
    ...
    _deactivatePasteMode(_callback) {
        if (window.pasteMode.activated) {
            ...
            this.$xNoPeers.querySelector('h2').innerHTML = 'Open ShareGlimmer on other devices to send files';
    ...
    async _displayFiles(peerId, displayName, connectionHash, files, imagesOnly, totalSize) {
        ...
        if (files.length > 1) {
            ...
            try {
                ...
                filenameDownload = `ShareGlimmer_files_${year+month+date}_${hours+minutes}.zip`;
            } catch (e) {
                console.error(e);
        ...
        document.title = files.length === 1
            ? 'File received - ShareGlimmer'
            : `${files.length} Files received - ShareGlimmer`;
...
class ReceiveRequestDialog extends ReceiveDialog {
    ...
    _showRequestDialog(request, peerId) {
        ...
        document.title = `${request.imagesOnly ? 'Image' : 'File'} Transfer Requested - ShareGlimmer`;
        document.changeFavicon("images/favicon-96x96-notification.png");
        this.show();
    }
...
class ReceiveTextDialog extends Dialog {
    ...
    _setDocumentTitleMessages() {
        document.title = !this._receiveTextQueue.length
            ? 'Message Received - ShareGlimmer'
            : `${this._receiveTextQueue.length + 1} Messages Received - ShareGlimmer`;
    }
    ...
    constructor() {
        ...
        if (share_target_type) {
            if (share_target_type === "text") {
                ...
                let openRequest = window.indexedDB.open('ShareGlimmer_store')
...
class PersistentStorage {
    constructor() {
        ...
        const DBOpenRequest = window.indexedDB.open('ShareGlimmer_store', 4);
    ...
    static set(key, value) {
        return new Promise((resolve, reject) => {
            const DBOpenRequest = window.indexedDB.open('ShareGlimmer_store');
...
class BrowserTabsConnector {
    constructor() {
        this.bc = new BroadcastChannel('ShareGlimmer');
...
// close About ShareGlimmer page on Escape
